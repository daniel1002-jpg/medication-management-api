...
class PatientRepositoryPostgres extends IPatientRepository {
  ...
  async findById(id: string) {
    const result = await db.query("SELECT * FROM pacientes WHERE id = $1", [
      id,
    ]);
  ...
  async save(patientData: Patient): Promise<null | Patient> {
    const {
      domicilio,
      email,
      fecha_alta,
  ...
  async update(id: string, updateData: Partial<Patient>) {
    const result = await db.query(
      `UPDATE pacientes 
...
export default PatientRepositoryPostgres;
